(function () {
  ...
  /* The team roster lives in index.html. Keep names, images and ordering there. */
  var members = Array.prototype.slice.call(document.querySelectorAll('.member'));

  /* Display positions are based on the actual card order in the HTML. */
  members.forEach(function (card, index) {
    var tag = card.querySelector('.badge');
    if (tag) tag.textContent = 'Посада ' + (index + 1);
  });

  var arina = members.find(function (card) {
    var title = card.querySelector('.member__name');
    return title && title.textContent.trim() === 'Болдирєва Аріна';
  });
  if (arina) {
